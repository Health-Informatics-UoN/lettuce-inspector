...
## Pipelines
Retrieval: BGE-small with short embedding strings, top 10
...
db = duckdb.connect()
connect_to_vector_parquet(db, "~/embeddings-storage/bge-small.parquet")
...
tests = [
        ...
        *[
            RAGPipelineTest(
                f"{name} RAG with BGE-small (concept name only in prompt)",
                pipeline,
                generation_metrics,
                )
        ...
        *[
            RAGPipelineTest(
                f"{name} RAG with BGE-small (concept name and score in prompt)",
                pipeline,
                generation_metrics,
                )
        ...
        *[
            RAGPipelineTest(
                f"{name} RAG with BGE-small (concept attributes in prompt)",
                pipeline,
                generation_metrics,
                )

...
description = """
Testing BGE-small for the Lettuce paper

Dataset: small test

## Pipelines
Retrieval: BGE-small with long embedding strings, top 10
Generation: BGE-small for RAG
    - llama 3.2 3B
    - llama 3.1 8B
    - phi-4
    - Qwen2.5 14b
...
db = duckdb.connect()
connect_to_vector_parquet(db, "~/embeddings-storage/bge-small-long.parquet")
connect_to_concept_csv(db, "~/vocabs/CONCEPT.csv")

logger.info("Initialising embedding model")
bge_small = SentenceTransformer("BAAI/bge-small-en-v1.5", device="cuda")
...
pq = ParquetFileVectorSearcher(
        ...
        model=bge_small,
        vocabulary_ids=None,
        vector_dimension=bge_small.get_sentence_embedding_dimension()
        )
bge_small_retriever = EmbeddingsRetrievalPipeline(
...
tests = [
        InformationRetrievalPipelineTest("BGE-small retrieval", bge_small_retriever, retrieval_metrics),
        *[
            RAGPipelineTest(
                f"{name} RAG with BGE-small (long embedding strings, concept name only in prompt)",
                pipeline,
                generation_metrics,
                )
        ...
        *[
            RAGPipelineTest(
                f"{name} RAG with BGE-small (long embedding strings, concept name and score in prompt)",
                pipeline,
                generation_metrics,
                )
        ...
        *[
            RAGPipelineTest(
                f"{name} RAG with BGE-small (long embedding strings, concept attributes in prompt)",
                pipeline,
                generation_metrics,
                )
...
framework = EvaluationFramework(
        name = "BGE-small long test NUH data",

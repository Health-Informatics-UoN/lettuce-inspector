...
logger = logging.Logger("pubmedbert-test")
...
## Pipelines
Retrieval: pubmedbert with long embedding strings, top 10
...
db = duckdb.connect()
connect_to_vector_parquet(db, "~/embeddings-storage/pubmedbert-long.parquet")
...
tests = [
        InformationRetrievalPipelineTest("pubmedbert retrieval (long strings)", pubmedbert_retriever, retrieval_metrics),
        *[
            RAGPipelineTest(
                f"{name} RAG with pubmedbert (long strings, concept name only in prompt)",
                pipeline,
                generation_metrics,
                )
        ...
        *[
            RAGPipelineTest(
                f"{name} RAG with pubmedbert (long strings, concept name and score in prompt)",
                pipeline,
                generation_metrics,
                )
        ...
        *[
            RAGPipelineTest(
                f"{name} RAG with pubmedbert (long strings, concept attributes in prompt)",
                pipeline,
                generation_metrics,
                )
...
framework = EvaluationFramework(
        name = "pubmedbert (long) test NUH data",

...
from query_handler.duckdb_connectors import connect_to_vector_parquet
...
{% for result in vec_results %}
    {{result.concept_id}} (score = {{1-result.score|round(3)}})
{% endfor %}
...
tests = [
        InformationRetrievalPipelineTest("BGE-small (long strings)", pubmedbert_retriever, retrieval_metrics),
        *[
            RAGPipelineTest(
                f"{name} RAG with BGE-small (long strings)",
                pipeline,
                generation_metrics,
                )
...
framework = EvaluationFramework(
        name = "BGE-small test (long strings)",

...
from evaluation.pipelines import LLMChatPipeline, RAGChatPipeline
...
llm_system_prompt = """You are an assistant that suggests formal RxNorm names for a medication. You will be given the name of a medication, along with some possibly related RxNorm terms. If you do not think these terms are related, ignore them when making your suggestion.
...
rag_prompt_template_with_scores = template_env.from_string(
    """Possible related terms:
{% for result in vec_results %}
    {{result.content}} (score = {{1-result.score}})
{% endfor %}
...
llm_only = LLMChatPipeline(llm=llm,
...
rag_5_with_score = RAGChatPipeline(
    llm=llm,
...
rag_10_with_score = RAGChatPipeline(
    llm=llm,
    system_prompt=rag_system_prompt,
    prompt_template=rag_prompt_template_with_scores,
    template_vars=template_vars,
...
tests = [
    ...
    RAGPipelineTest("20 matches+scores", rag_10_with_score, metrics),
]
